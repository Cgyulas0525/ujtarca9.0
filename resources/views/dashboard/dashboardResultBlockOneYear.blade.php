    ...
    <div class="card-footer p-0">
        <ul class="nav flex-column">
            ...
            @include('dashboard.dashboardResultBlockItem', ['title' => 'Egyenleg',
                                                            'route' => 'invoices.index',
                                                            'value' => number_format(DashboardController::financialResultThisYear([date('Y') - 1]),0,",","."),
                                                            'class' => 'bg-danger'])
            @include('dashboard.dashboardResultBlockItem', ['title' => 'Készpénz',
                                                            'route' => 'invoices.index',
                                                            'value' => number_format(DashboardController::cashThisYear(date('Y-m-d', strtotime('January 1 -1 year')),
                                                                                                                       date('Y-m-d', strtotime('December 31 -1 year'))),0,",","."),
                                                            'class' => 'bg-primary'])
            @include('dashboard.dashboardResultBlockItem', ['title' => 'Bankkártya',
                                                            'route' => 'invoices.index',
                                                            'value' => number_format(DashboardController::cardThisYear(date('Y-m-d', strtotime('January 1 -1 year')),
                                                                                                                       date('Y-m-d', strtotime('December 31 -1 year'))),0,",","."),
                                                            'class' => 'bg-primary'])
            @include('dashboard.dashboardResultBlockItem', ['title' => 'Szép kártya',
                                                            'route' => 'invoices.index',
                                                            'value' => number_format(DashboardController::szcardThisYear(date('Y-m-d', strtotime('January 1 -1 year')),
                                                                                                                       date('Y-m-d', strtotime('December 31 -1 year'))),0,",","."),
                                                            'class' => 'bg-primary'])
            @include('dashboard.dashboardResultBlockItem', ['title' => 'Napi átlag',
                                                            'route' => 'invoices.index',
                                                            'value' => number_format(DashboardController::averigeThisYear(date('Y-m-d', strtotime('January 1 -1 year')),
                                                                                                                       date('Y-m-d', strtotime('December 31 -1 year'))),0,",","."),
                                                            'class' => 'bg-danger'])
        </ul>
    </div>
